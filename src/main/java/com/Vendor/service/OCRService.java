...
public class OCRService {
    ...
    public OCRResult scanDocument(MultipartFile file) {
        ...
        try {
            ...
            if (contentType == null) throw new RuntimeException("Invalid file");
            ...
            if (contentType.startsWith("image/")) {
                ...
                File temp = File.createTempFile("ocr_" + UUID.randomUUID(), ext);
            ...
            // ===== PDF =====
                ...
                PDDocument doc = PDDocument.load(tempPdf);
    ...
    private OCRResult buildResult(String text) {
        ...
        if (text.contains("INCOME TAX")) {
            ...
            result.setStatus(result.getPan().equals("NOT FOUND") ? "REJECTED" : "VERIFIED");
        }
            ...
            result.setStatus(result.getAadhaar().equals("NOT FOUND") ? "REJECTED" : "VERIFIED");
        }
            ...
            result.setStatus(result.getAccountNumber().equals("NOT FOUND") ? "REVIEW" : "VERIFIED");
        }
    ...
    private String extractAadhaar(String text) {
        Matcher m = Pattern.compile("\\d{4}\\s?\\d{4}\\s?\\d{4}").matcher(text);
        return m.find() ? m.group().replaceAll("\\s", "") : "NOT FOUND";
    }

    private String extractAccount(String text) {
        Matcher m = Pattern.compile("\\d{9,18}").matcher(text);
        return m.find() ? m.group() : "NOT FOUND";
    }
    ...
    private String extractName(String text) {
        ...
        for (String line : lines) {
            ...
            if (line.contains("INCOME") || line.contains("GOVERNMENT") || line.contains("INDIA"))
                continue;
